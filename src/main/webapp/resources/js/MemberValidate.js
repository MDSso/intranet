...
function validate(formid) {
	   ...
	   if(etDate.value == ""){
		...
		if(!re.test(memId.value)){
			modal("아이디는 영문,숫자 3~15자로 입력해 주세요.","one_btn_modal","memId");
			return false;
		}
		
		if(re.test(memId.value)){
			if(idCheck("memId")){
				modal("아이디가 중복됩니다.","one_btn_modal","memId");
				return false;
			}
		}
		
		if(!re3.test(memPw.value)){
			modal("비밀번호는 특수문자, 숫자, 문자를 포함한 8~20자로 입력해 주세요.","one_btn_modal","memPw");
			return false;
		}
		
		if(memPw.value != confirmPw.value){
			modal("비밀번호가 일치하지 않습니다.","one_btn_modal","confirmPw");
			return false;
		}
